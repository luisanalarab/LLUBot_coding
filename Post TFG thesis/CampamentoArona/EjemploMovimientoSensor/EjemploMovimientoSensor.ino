...
void loop() {
  ...
  if (sensorVal == 0) {
    ...
    if (request.indexOf("/MOTOR=DANCE") != -1) {
      value = 5;
      request.remove(0,pos+1);
      mov = request.toInt();
    } 
    ...
    client.println("  </head>");
    client.println("  <body>");
    client.println("    <h1> LLUBot</h1>");
//    client.println("    <p> Vamos a mover en equipos el LLUBot hasta la meta amarilla.</p>");
    client.println("    <button class= 'buttonforward'> <a href=\"/MOTOR=DANCE\"> Baila! </a></button><br></br>");
